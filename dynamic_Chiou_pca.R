dynamic_Chiou_pca = function(dat, newdata, holdoutdata, order_k_percent = 0.9, order_m_percent = 0.9, 
...
err_comp <- function(dat, ik, bootrep)
{
    err = matrix(, (nrow(dat) - ik), (ncol(dat) - 2))
    for(j in 2:(ncol(dat)-1))
    {
        holdout = dat[(ik+1):nrow(dat),(j+1)]
        dum = dynamic_Chiou(dat = dat[,1:j], newdata = matrix(dat[1:ik, (j+1)], ncol = ik), 
                            holdoutdata = matrix(holdout, ncol = nrow(dat) - ik),
